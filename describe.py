...
def compute_tfidf_descriptors(
    path_or_dataframe: Union[Path, pd.DataFrame],
    ...
    df_grouped = _collect_text_from_column(
        df_, cluster_label_column, "docs", text_column
    )
    ...
    try:
        ...
        logger.warn(
            f"Encountered error: {error}. Will proceed with different vectorizer values."
        )
    ...
    df_grouped["max_descriptors"] = [
        [
            (reverse_vocabulary.get(item), word2idf.get(reverse_vocabulary.get(item)))
            for item in row
        ]
        for row in tfidf_descriptors
    ]

    df_grouped["tfidf_descriptors"] = df_grouped["max_descriptors"].apply(
        _collapse_on_prefixes
    )
    df_grouped["tfidf_descriptors"] = df_grouped["tfidf_descriptors"].apply(
        _collapse_on_suffixes
    )
    df_grouped["tfidf_descriptors"] = df_grouped["tfidf_descriptors"].apply(
        _clean_up_descriptors
    )
    df_grouped.drop(
        columns=["title_normalized", "docs", "max_descriptors"], inplace=True
    )
    return df_grouped
...
def compute_yake_descriptors(
    ...
    df_grouped["yake_descriptors"] = df_grouped["docs"].apply(
        extractor.extract_keywords
    )
    df_grouped["yake_descriptors"] = df_grouped["yake_descriptors"].apply(
        _collapse_on_prefixes
    )
    df_grouped["yake_descriptors"] = df_grouped["yake_descriptors"].apply(
        _collapse_on_suffixes
...
def _filter_on_cluster_prob(
    df: pd.DataFrame, cluster_probability_column: str, cluster_probability: float
) -> pd.DataFrame:
    ...
    if cluster_probability:
        if cluster_probability_column not in df.columns:
            print(
                f"Cannot filter on {cluster_probability_column}: not in columns {df.columns}"
            )
            df_ = df.copy()
        else:
...
def _collect_text_from_column(
    df: pd.DataFrame, source_column: str, target_column: str, text_column: str
) -> pd.DataFrame:
...
def compute_cohere_descriptors(
    path_or_dataframe: Union[Path, pd.DataFrame],
    ...
    if not cohere_api_key:
        ...
        if not cohere_api_key:
            logger.warn(
                "No API key given for Cohere service. Skipping computing of Cohere cluster descriptors."
            )
            return None
    ...
    logger.info(
        f"Data frame has {df.shape[0]} rows after thresholding on cluster probability {cluster_probability}"
    )
    df = df[df[cluster_label_column] > -1]
    texts = df[text_column].tolist()
    clusters = df[cluster_label_column].tolist()
    logger.info(
        f"Will use {len(texts)} texts from which Cohere will select {num_sample_texts} for each cluster"
    )

    topic_names = app.name_topics(
        texts,
        clusters,
    ...
    if topic_names is not None:
        df_ = pd.DataFrame(
            [
                {"cluster_label": label, "cohere_descriptor": desc}
                for label, desc, in topic_names.items()
            ]
        )
...
def describe(
    ...
    tfidf_desc_df = compute_tfidf_descriptors(
        df, cluster_probability=cluster_probability
    )
    yake_desc_df = compute_yake_descriptors(df, cluster_probability=cluster_probability)
    cohere_desc_df = compute_cohere_descriptors(
        df, cluster_probability=cluster_probability
    )
